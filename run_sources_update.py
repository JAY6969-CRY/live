#!/usr/bin/env python
"""
Run NewsSense with enhanced source link display.
This script creates a simplified version focusing on source links display.
"""
import os
import sys
import subprocess
...
def run_with_sources():
    """Run the simplified app with enhanced source links."""
    print("\nStarting Streamlit UI with Enhanced Source Links...")
    # Add the current directory to the Python path
    env = os.environ.copy()
    env["PYTHONPATH"] = os.getcwd() + os.pathsep + env.get("PYTHONPATH", "")
    
    # Create a simplified app file focusing on source links
    with open("temp_sources_app.py", "w", encoding="utf-8") as f:
        f.write('''
...
''')
    ...
    process = subprocess.Popen(
        ["streamlit", "run", "temp_sources_app.py", "--server.port=8501"],
        env=env
    )
    ...
    print("\nStreamlit UI is running with Enhanced Source Links.")
    print("This demonstration shows how source links can be displayed with source logos,")
    print("direct links to original news sources, and integrated stock performance data.")
    ...
    try:
        ...
        if os.path.exists("temp_sources_app.py"):
            os.remove("temp_sources_app.py")
